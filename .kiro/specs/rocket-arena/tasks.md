...
- The arena boundary shell was repainted on request, entirely in materials. No collision geometry, primitive, or seam was touched, so the resolved geometry fingerprint, `shared/tests/arena-collision.test.ts`, and the renderer boundary metadata are all unchanged. The defect was concrete: `field-containment` was a `MeshPhysicalMaterial` at `opacity 0.2` with `depthWrite` disabled, so the wall occluded nothing and the crowd, trusses, skyline, and sky all read straight through it; a car on the wall had no visible surface under it to judge height or position against. The wall is opaque now, with panel seams spaced against the arena's own length, evenly spaced rows, a bright kick plate at the ramp, a cool cap at the ceiling curve, and one continuous bright team stripe around the perimeter. A deep team tint alone was tried first and was invisible, because a dark tint mixed into a dark panel is not a tint. The `field-lower-transition` band is painted the same way so the start of the climb is visible rather than discovered, and the two goal interiors, previously near-black and within a few points of each other, now carry enough of their own team to be told apart. Implementation notes: the pattern is injected into `MeshStandardMaterial` via `onBeforeCompile` so the wall still takes the stadium lighting instead of reading as a flat sheet, `customProgramCacheKey` is keyed on the source, arena dimensions are baked in as GLSL literals since they are fixed for the material's life, and both transition risers are read from `resolvedGeometry.profiles` rather than from a local copy of the numbers. The ceiling stays glass and is now the only boundary held back for transparency sorting.
